import { NextResponse } from 'next/server';

const LimitInquiryResponse_DetailedDummyData = {
    id: 409713573,
    insertTime: 1726185641000,
    applicationId: 334414,
    product: {
        id: 926005,
        bank: {
            id: 21,
            name: '신한저축은행',
            bankLosgoUrl: 'https://cdn.finda.co.kr/images/bank/app/199_4.png',
            backgroundBankLogo: 'https://cdn.finda.co.kr/images/bank/app/199_4_bg.png',
            bankDivisionCode: '05',
            bankIntroduction:
                '신한금융지주 산하 저축은행으로 2020년 7월부터 핀다를 통해 고객들에게 대출을 실행하고 있습니다.',
            displayOrder: 150,
            isUnderInspection: false,
        },
        name: '햇살론직장인',
        category: 'credit',
        displayProperty: {
            v1: {
                rateType: ['flexible'],
                tag: [''],
                productTag: [],
                bridge: {
                    type: 'APP',
                    bridgeImgLink: '',
                    bridgeFullImgLink:
                        'https://cdn.finda.co.kr/images/bank/app/loanapply/bridgeimg/android/bank_bridgeimg_shinhan_sb.png',
                },
                informMessage:
                    '- 금융사 대출실행 가능시간에만 계약을 진행하실 수 있습니다.  \n  : 평일 오전 9시~오후 6시 30분 / 주말·공휴일 불가  \n- 계약 관련 문의사항이 있으신가요?   \n  신한저축은행 고객센터 : 1644-5187',
                businessHour: '신한저축은행: 월 ~ 금 (09:00 ~ 18:30)',
                paymentMethod: {
                    repayAllAtEnd: {
                        bool: false,
                        months: [12, 24, 36, 48, 60],
                    },
                    repaySameRateAmount: {
                        bool: false,
                        months: [36, 60],
                    },
                    repaySameAmount: {
                        bool: true,
                        months: [36, 60],
                    },
                },
                promotionCode: '',
                bankPhoneNumber: '1644-5187',
                auditNumber: '준법감시인심의필 리테일2022-00577(22.08.01~23.07.31)',
                auditDate: '',
                auditExpDate: '',
                termsAndCondition: [
                    {
                        대출한도: '2,000만원까지',
                    },
                    {
                        대출대상:
                            '연소득 3,500만원 이하인 저소득자 이거나 연소득 4,500만원 이하이면서 1개사 이상의 개인신용평가사가 산정한 개인신용평점 하위 100분의 20에 해당하는 자 (3개월 이상 근로자) \n※ ‘22년 기준 NICE 744점, KCB 700점 이하 \n\n온라인햇살론 대상자 : 햇살론 직장인 대상자 중 \n① 현 직장 최근 건강보험료 3개월 이상 정상 납부 \n② 공동인증서, 본인명의 휴대폰보유 \n ※ 온라인햇살론을 신청할 경우 기존 햇살론 대비 금리가 1.3%p 인하됩니다.',
                    },
                    {
                        대출금리:
                            '연 7~10% 대 (매월 변동) \n※ 매월 금융감독원에서 통보하는 상한금리 이내에서 금융회사가 자율적으로 결정 (최대 10.5% 이내)',
                    },
                    {
                        이자부과시기: '매월 납입일에 후취',
                    },
                    {
                        대출부대비용:
                            '*보증비용 \n· 보증요율 연 2% 이내 \n  ① 저소득(연 35백만원 이하) 청년(만 34세 이하) 0.5%p 인하 \n  ② 사회적배려대상 및 근로장려금 수급자 · 자활근로자는 연 1%p 인하 \n※ ①,②항목 중복인하 불가 \n\n* 사회적배려대상 : 「한부모가족지원법」 제 5조에 따른 한부모가족, 가구, 동법 제5조의2 제2항에 따른 조손가족 가구, 「다문화가족지원법」 제2조에 따른 다문화가족가구, 「북한이탈주민의 보호 및 정착지원에 관한 법률」 제2조제1호에 따른 북한이탈주민, 「장애인복지법」 제32조에 따른 등록 장애인, 「국민기초생활보장법」 제2조 제2호에 따른 국민기초생활수급자, 동법 제2조 제10호에 따른 차상위계층',
                    },
                    {
                        대출기간: '3년 또는 5년',
                    },
                    {
                        중도상환수수료: '없음',
                    },
                    {
                        연체금리: '약정이율 + 연 3% (법정최고금리 연 14% 이내)',
                    },
                    {
                        상환방법: '원금균등분할상환',
                    },
                    {
                        유의사항:
                            '· 과도한 대출은 개인신용평점 하락의 원인이 될 수 있습니다. \n · 신용등급 또는 개인신용평점 하락으로 금융거래의 제약 또는 불이익이 발생할 수 있습니다. \n · 연체 시 계약기한 만료 전 원리금 변제의무가 발생할 수 있습니다. \n · 대출한도 및 금리는 신용평점에 따라 차등 적용됩니다.  \n · 당사는 다수의 대출상품이 존재하며 상품별로 대상, 한도, 금리가 다릅니다. 상담과정에서 고객님께 적합한 상품을 안내해드립니다. \n · 대출심사에 필요한 서류의 제출을 요구할 수 있고, 신용평점, 소득, 금융거래 상황 등 심사기준에 따라 심사결과가 달라지거나 대출이 거절될 수 있습니다. \n ·  본인의 재무상태 개선 또는 신용도가 상승하였다고 판단되는 경우 금리인하를 요구할 수 있습니다. 단, 차주의 신용상태와 관계없이 금리가 결정되는 대출상품(스탁론, 집단대출, 예적금담보대출 등)과 무연체 기간에 따라 자동으로 금리 인하가 적용되는 햇살론 상품은 제외되며, 심사결과에 따라 대출금리가 인하되지 않을 수 있습니다.  \n · 일반금융소비자는 대출계약체결일 익일부터 14일 이내에 대출계약에 대한 청약을 철회할 수 있으며, 이미 공급받은 금전·재화와 그에 대한 이자 및 수수료는 반환하여야 합니다. 또한 철회권을 행사하여 효력이 발생한 이후에는 이를 취소할 수 없습니다. \n · 금융회사가 판매원칙을 위반하여 금융상품 계약을 체결한 경우 계약일로부터 5년, 위법사실을 안 날로부터 1년 계약의 해지를 요구할 수 있습니다. (계약종료시 행사 불가) \n · 금융소비자는 금융소비자보호법 제36조에 따른 분쟁조정 또는 소송의 수행 등 권리구제를 위한 목적으로 자료의 열람(사본의 제공 또는 청취 포함)을 요구할 수 있습니다. \n · 금융소비자보호법 제19조제1항에 따라 해당 상품에 대해 충분한 사전 설명을 받을 권리가 있으며 설명을 이해한 후 거래하시기 바랍니다. \n · 계약을 체결하기 전 상품설명서 및 여신거래기본약관을 확인하시기 바랍니다. \n · 상품 가입 후 불만(민원)이 있을 경우 고객센터(1644-7777) 또는 인터넷 홈페이지(www.shinhansavings.com) 로 문의하실 수 있으며 분쟁이 발생한 경우에는 금융감독원(국번없이 1332) 등에 도움을 요청하실 수 있습니다.',
                    },
                ],
            },
        },
        majorCategory: 'CREDIT_LOAN',
        minorCategory: 'SUNSHINE_LOAN',
        requiredOpenAccount: false,
        redemptionFeeYn: false,
        expectedHours: 1,
        expectedMins: 0,
        productCode: '52301',
        contractApplyCount: 1,
        tags: [
            {
                type: 'AVAILABLE_TODAY_DEPOSIT',
                text: '오늘입금',
            },
        ],
    },
    condition: {
        id: 409713573,
        loanLimit: 20000000,
        findaPreferredLoanLimit: 0,
        interestType: 'VARIABLE',
        loanRate: 6.52,
        baseRate: 6.52,
        findaPreferredLoanRate: 0,
        loanPeriod: 60,
        repayMethod: 'LEVELPRINCIPLE',
        loanReliabilityGrade: 'mid',
    },
    status: 'condition_approved',
    contractType: 'OUTBOUND_CALL',
    responseMessage: '정상',
};

export async function GET() {
    return NextResponse.json(LimitInquiryResponse_DetailedDummyData);
}
